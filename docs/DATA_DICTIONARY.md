...
## 高成數按揭 Manifest（`mortgage_manifest`）

> 記錄從 HKMC 下載嘅按揭保險準則及保費 PDF 清單。原始 PDF 存放於 `data/downloads/hkmc/`。

| 欄位 | 類型 | 必填 | 說明 | 示例 |
|------|------|------|------|------|
| `resource_id` | TEXT | ✓ | 資源識別碼 | `premium_private_chi` |
| `category` | TEXT | ✓ | 類別：`guideline`（準則）/ `premium`（保費） | premium |
| `name` | TEXT | ✓ | 文件名稱 | 按揭保險計劃按揭保費一覽表 |
| `description` | TEXT | | 簡要說明 | 涵蓋70%-90% LTV 保費率 |
| `url` | TEXT | ✓ | 官方下載 URL | https://www.hkmc.com.hk/... |
| `local_path` | TEXT | ✓ | 本地存放路徑 | data/downloads/hkmc/premium_private_chi.pdf |
| `downloaded_at` | DATETIME | | 下載時間（ISO 8601） | 2026-08-05T02:00:00+00:00 |
| `file_size_bytes` | INTEGER | | 檔案大小（bytes） | 524288 |
| `sha256` | TEXT | | 檔案 SHA-256 校驗碼 | a1b2c3... |
| `status` | TEXT | ✓ | `downloaded` / `skipped` / `failed` | downloaded |
| `error` | TEXT | | 失敗原因（如有） | |
