...
from src.ingestion.providers.registry import monthly_fetchers
...
def download_tuen_mun_multi_source(months_back: int = 6) -> tuple[dict[str, Path], list[str], int]:
    ...
    for source_id, (label, provider) in monthly_fetchers().items():
        try:
            transactions = provider.fetch_monthly(months_back=months_back, enrich_agents=True)
            if not transactions:
